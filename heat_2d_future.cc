...
typedef struct update_task_args_s{
  int start;
  int end;
  int ny;
  float *u1;
...
void update_task(const void *args, size_t arglen,
              const void *userdata, size_t userlen, Realm::Processor p)
{
  int rank;
  MPI_Comm_rank(MPI_COMM_WORLD,&rank);
//  printf("rank %d, update task\n", rank);
  const update_task_args_t *update_task_args = (const update_task_args_t*)args;
  int start = update_task_args->start;
  int end = update_task_args->end;
  int ny = update_task_args->ny;
  float *u1 = update_task_args->u1;
  float *u2 = update_task_args->u2;
  update(start, end, ny, u1, u2);
}


void heat(const void *args, size_t arglen,
          const void *userdata, size_t userlen, Realm::Processor p)
{
  ...
  /* First, find out my taskid and how many tasks are running */
     ...
     if (taskid == MASTER) {
       double start_time = get_cur_time();
        ...
        inidat(NXPROB, NYPROB, (float*)u);
        //prtdat(NXPROB, NYPROB, (float*)u, "initial.dat");
     ...
     if (taskid != MASTER) 
     {
        ...
        iz = 0;
        MPI_Future sendr_future;
        MPI_Future recvr_future;
        MPI_Future sendl_future;
        MPI_Future recvl_future;
        MPI_Future update_future;
        MPI_Future comp_start_future;
        for (it = 1; it <= STEPS; it++)
        {
           if (left != NONE)
           {
             
              MPI_Fsend(&u[iz][offset][0], NYPROB, MPI_FLOAT, left,
                       RTAG, MPI_COMM_WORLD, &update_future, 1, &sendl_future);
              source = left;
              msgtype = LTAG;
              MPI_Frecv(&u[iz][offset-1][0], NYPROB, MPI_FLOAT, source,
                        msgtype, MPI_COMM_WORLD, &sendl_future, 1, &recvl_future);
              
              comp_start_future = recvl_future;
               /*         
           ...
           */
           ...
           if (right != NONE)
           {
              MPI_Fsend(&u[iz][offset+rows-1][0], NYPROB, MPI_FLOAT, right,
                        LTAG, MPI_COMM_WORLD, &recvl_future, 1, &sendr_future);
              source = right;
              msgtype = RTAG;
              MPI_Frecv(&u[iz][offset+rows][0], NYPROB, MPI_FLOAT, source, msgtype,
                        MPI_COMM_WORLD, &sendr_future, 1, &recvr_future);
              comp_start_future = recvr_future;
           }
           /* Now call update to update the value of grid points */
          // update(start,end,NYPROB,&u[iz][0][0],&u[1-iz][0][0]);
           update_task_args_t update_task_args;
           update_task_args.start = start;
           update_task_args.end = end;
           update_task_args.ny = NYPROB;
           update_task_args.u1 = &u[iz][0][0];
           update_task_args.u2 = &u[1-iz][0][0];
           MPI_Fexecute(update_task, &update_task_args, sizeof(update_task_args), &comp_start_future, 1, &update_future);
           iz = 1 - iz;
           
        }
        MPI_Waitfuture(&update_future);
